...
def device_sync(device):
    if "cuda" in device:
        ...
        import logging
        logging.warning(f"device={device} is not yet suppported")
...
def _load_model(checkpoint_path, device, precision, use_tp):
    ...
    if "int4" in str(checkpoint_path):
        logging.info("Using int4 weight-only quantization!")
...
def main(
    ...
    for i in range(start, num_samples):
        ...
        if not interactive:
            logging.info(tokenizer.decode(y.tolist()))
        else:
            logging.info("")
        tokens_generated = y.size(0) - prompt_length
        tokens_sec = tokens_generated / t
        aggregate_metrics['tokens_per_sec'].append(tokens_sec)
        logging.info(f"Time for inference {i + 1}: {t:.02f} sec total, {tokens_sec:.02f} tokens/sec")
    ...
    if is_speculative:
        ...
        logging.info(f"Mean Accepted: {sum([idx * i for idx, i in enumerate(counts_aggregated)])/sum(counts_aggregated)}")

    logging.info(f"Average tokens/sec: {torch.mean(torch.tensor(aggregate_metrics['tokens_per_sec'])).item():.2f}")
