...
try:
    ...
    # GPTQ and eval modules are optional dependencies
    GenericGPTQRunner = None
    InputRecorder = None
    get_task_dict = None
    evaluate = None
    lm_eval = None
...
class GPTQQuantHandler(QuantHandler):
    ...
    def get_inputs(model, tokenizer, calibration_tasks, calibration_limit, calibration_seq_length, pad_calibration_inputs) -> "MultiInput":
        ...
        try:
            ...
            # lm_eval may not be available or tasks initialization may fail
            # This is acceptable as it's an optional dependency
            pass
        # 使用标准logging，因为这个模块可能被其他地方导入
        import logging
        task_dict = get_task_dict(calibration_tasks)
        logging.info(f"Obtaining GPTQ calibration inputs on: {calibration_tasks}")
    ...
    def create_quantized_state_dict(self, use_cuda = True):
        ...
        for fqn, mod in self.mod.named_modules():
            if isinstance(mod, torch.nn.Linear):
                ...
                logging.debug(f"linear: {fqn}, in={in_features}, out={out_features}")
                ...
                if not _check_linear_int4_k(in_features, self.groupsize, self.inner_k_tiles):
                    if self.padding:
                        ...
                        logging.warning(f"warning: {fqn} is padded to satisfy in_features % 1024 == 0")
                        padded_in_features = find_multiple(in_features, 1024)
                        weight = F.pad(weight, pad=(0, padded_in_features - in_features))
                    else:
                        logging.warning(f"warning: {fqn} is skipped, int4 requires that in_features is 32, 64, or is divisible by 1024, " +
...
def quantize(
    ...
    if mode == 'int8':
        logging.info("Quantizing model weights for int8 weight-only symmetric per-channel quantization")
        ...
        logging.info("Quantizing model weights for int4 weight-only affine per-channel groupwise quantization")
        ...
        logging.info("Quantizing model weights for int4 weight-only affine per-channel groupwise quantization using GPTQ...")
