...
# 配置日志
logger = setup_logging("tts.seedvc_server.modules.openvoice.se_extractor", log_to_file=False)
...
def split_audio_vad(audio_path, audio_name, target_dir, split_seconds=10.0):
    ...
    logger.debug(f"VAD segments: {segments}")
...
def get_se(audio_path, vc_model, target_dir='processed', vad=True):
    ...
    logger.debug(f"OpenVoice version: {version}")
