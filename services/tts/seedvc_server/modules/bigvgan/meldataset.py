...
def mel_spectrogram(
    ...
    import logging
    if torch.min(y) < -1.0:
        logging.warning(f"[WARNING] Min value of input waveform signal is {torch.min(y)}")
    if torch.max(y) > 1.0:
        logging.warning(f"[WARNING] Max value of input waveform signal is {torch.max(y)}")
...
def get_dataset_filelist(a):
    ...
    with open(a.input_training_file, "r", encoding="utf-8") as fi:
        ...
        import logging
        logging.debug(f"first training file: {training_files[0]}")
    ...
    with open(a.input_validation_file, "r", encoding="utf-8") as fi:
        ...
        import logging
        logging.debug(f"first validation file: {validation_files[0]}")
...
class MelDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        import logging
        logging.info("[INFO] checking dataset integrity...")
