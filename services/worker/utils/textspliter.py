...
class TextSplitter(ABC):
    ...
    def __init__(
        ...
        if chunk_overlap > chunk_size:
            ...
            e.error_detail = f"Got a larger chunk overlap ({chunk_overlap}) than chunk size " f"({chunk_size}), should be smaller."
            raise e
...
class MarkdownHeaderTextSplitter:
    ...
    def _process_header_line(
        ...
        """Process a line that contains a header.
        
        Args:
            line: Original line
            stripped_line: Stripped line content
            current_content: Current content being accumulated
            current_metadata: Current metadata
            initial_metadata: Initial metadata for current context
            header_stack: Stack of headers being tracked
            lines_with_metadata: List to store processed lines
            
        Returns:
            True if line was processed as header, False otherwise
        """
        for sep, name in self.headers_to_split_on:
            if stripped_line.startswith(sep) and (
                ...
                if name is not None:
                    self._update_header_stack(
                        stripped_line, sep, name, header_stack, initial_metadata
                    )
                
                if current_content:
                    self._finalize_current_content(
                        current_content, current_metadata, lines_with_metadata
                    )
                return True
        return False
    ...
    def _update_header_stack(
        ...
        """Update the header stack when a new header is encountered."""
    ...
    def _finalize_current_content(
        self,
        current_content: List[str],
        current_metadata: Dict[str, str],
        lines_with_metadata: List[LineType]
    ) -> None:
        """Add the current content to lines_with_metadata and clear it."""
    ...
    def _process_content_line(
        ...
        """Process a line that contains regular content."""
        if stripped_line:
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        for line in lines:
            ...
            # Try to process as header line first
            is_header = self._process_header_line(
                line, stripped_line, current_content, current_metadata,
                initial_metadata, header_stack, lines_with_metadata
            )
            
            # If not a header, process as content line
            if not is_header:
                self._process_content_line(
                    stripped_line, current_content, current_metadata, lines_with_metadata
                )
        ...
        # Return either individual lines or aggregated chunks
...
def general_split_text(text: str, rules: dict) -> list:
    ...
    elif split_method == "chapter":
        paragraphs = re.split(r'(第[\d一二三四五六七八九十百千]+章[\s\S]*?)(?=第[\d一二三四五六七八九十百千]+章|$)', text)
